...
groundColor = (100, 100, 100)  # Color to represent the ground
...
pygame.display.set_caption("Dino Run - Project ")

# Base path for resources
base_path = 'resources'

# Load sound effects
soundOnJump = pygame.mixer.Sound(os.path.join(base_path, 'jump.wav'))
soundOnDie = pygame.mixer.Sound(os.path.join(base_path, 'die.wav'))
soundOnCheckpoint = pygame.mixer.Sound(os.path.join(base_path, 'checkPoint.wav'))

def load_image(name, sx=-1, sy=-1, colorkey=None):
    img = pygame.image.load(os.path.join(base_path, name))
    img = img.convert()
...
def load_sprite_sheet(s_name, namex, namey, scx=-1, scy=-1, c_key=None):
    sh = pygame.image.load(os.path.join(base_path, s_name))
    sh = sh.convert()
    sh_rect = sh.get_rect()
...
class Score(pygame.sprite.Sprite):
    def __init__(self, sx=20, sy=30):
        pygame.sprite.Sprite.__init__(self, self.containers)
        self.sprites, self.rect = load_sprite_sheet('numbers.png', 10, 1, sx, sy, white_color)
        self.score = 0

    def draw(self):
        score_digits = extractDigits(self.score)
        x = width_screen / 2 - (len(score_digits) * 10)
        for i in range(len(score_digits)):
            screenDisplay.blit(self.sprites[score_digits[i]], (x + i * 20, 15))
...
def main():
    ...
    scoreGroup = pygame.sprite.GroupSingle()
    score = Score()
    ...
    while running:
        ...
        pygame.draw.rect(screenDisplay, groundColor, pygame.Rect(0, int(0.98 * height_screen), width_screen, height_screen * 0.02))
        ...
        cactusGroup.update()
        cactusGroup.draw(screenDisplay)

        birdsGroup.update()
        birdsGroup.draw(screenDisplay)

        dino.update()
        dino.draw()

        scoreGroup.draw(screenDisplay)
